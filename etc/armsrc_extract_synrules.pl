...
my $latestDate = "";  # Latest datestamp found in provided MSS files
...
foreach my $mss (@inputFiles) {
  ...
  parse:
    ...
    if ($line =~ /^\@begin\{NotISO\}/i) {
      $notISO = 1;
    } elsif ($line =~ /\$Date: (\d\d\d\d\/\d\d\/\d\d) \d\d:\d\d:\d\d/) {
      my $cvsDate = $1;
      if ($cvsDate gt $latestDate) {
        $latestDate = $cvsDate;
      }
...
# Output
print "// Generated by armsrc_extract_synrules.pl version $Version\n";
print "// from MSS files of $latestDate\n\n";
